...
bool StyleSheetContents::wrapperInsertRule(GCPassPtr<StyleRuleBase> rule, unsigned index)
{
    ...
    if (index < m_importRules.size() || (index == m_importRules.size() && rule.get()->isImportRule())) {
        ...
        m_importRules.emplace(m_importRules.begin() + index, importRule);
    ...
    if (index < m_namespaceRules.size() || (index == m_namespaceRules.size() && rule.get()->isNamespaceRule())) {
        ...
        m_namespaceRules.emplace(m_namespaceRules.begin() + index, namespaceRule);
...
bool StyleSheetContents::wrapperDeleteRule(unsigned index)
{
    ...
    if (index < m_namespaceRules.size()) {
        if (!m_childRules.empty())
            return false;
        m_namespaceRules.erase(m_namespaceRules.begin() + index);
        return true;
    }
    index -= m_namespaceRules.size();
...
void StyleSheetContents::startLoadingDynamicSheet()
{
    ...
    ASSERT(false); // BKTODO: copyToVector(root->m_completedClients, completedClients);
...
static void removeFontFaceRules(WillBeHeapHashSet<RawPtrWillBeWeakMember<CSSStyleSheet>>& clients, const StyleRuleFontFace* fontFaceRule)
{
    for (const auto& sheet : clients) {
        if (Node* ownerNode = sheet->ownerNode())
            ASSERT(false); // BKTODO: ownerNode->document().styleEngine().removeFontFaceRules(std::vector<Member<const StyleRuleFontFace>>(1, fontFaceRule));
    }
}
...
static void findFontFaceRulesFromRules(const std::vector<GCMember<StyleRuleBase>>& rules, std::vector<GCMember<const StyleRuleFontFace>>& fontFaceRules)
{
    for (unsigned i = 0; i < rules.size(); ++i) {
        ...
        if (rule->isFontFaceRule()) {
            fontFaceRules.emplace_back(toStyleRuleFontFace(rule));
