...
inline static GCRefPtr<CSSPrimitiveValue> zoomAdjustedNumberValue(double value, const ComputedStyle& style)
{
    return cssValuePool().createValue(value / style.effectiveZoom(), CSSPrimitiveValue::UnitType::Number);
}

static GCRefPtr<CSSPrimitiveValue> zoomAdjustedPixelValueForLength(const Length& length, const ComputedStyle& style)
{
    if (length.isFixed())
...
static GCRefPtr<CSSValueList> createPositionListForLayer(CSSPropertyID propertyID, const FillLayer& layer, const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> positionList = CSSValueList::createSpaceSeparated();
...
GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::currentColorOrValidColor(const ComputedStyle& style, const StyleColor& color)
{
    // This function does NOT look at visited information, so that computed style doesn't expose that.
    return cssValuePool().createColorValue(color.resolve(style.color()).rgb());
}

static GCRefPtr<CSSValue> valueForFillSize(const FillSize& fillSize, const ComputedStyle& style)
{
...
static GCRefPtr<CSSValue> valueForFillRepeat(EFillRepeat xRepeat, EFillRepeat yRepeat)
{
...
static GCRefPtr<CSSValue> valueForFillSourceType(EMaskSourceType type)
{
    switch (type) {
    case MaskAlpha:
...
static GCRefPtr<CSSValue> valueForPositionOffset(const ComputedStyle& style, CSSPropertyID propertyID, const LayoutObject* layoutObject)
{
    Length offset;
    switch (propertyID) {
...
static GCRefPtr<CSSBorderImageSliceValue> valueForNinePieceImageSlice(const NinePieceImage& image)
{
    // Create the slices.
    GCRefPtr<CSSPrimitiveValue> top, right, bottom, left;
...
static GCRefPtr<CSSQuadValue> valueForNinePieceImageQuad(const BorderImageLengthBox& box, const ComputedStyle& style)
{
    // Create the slices.
    GCRefPtr<CSSPrimitiveValue> top, right, bottom, left;
...
static GCRefPtr<CSSValue> valueForNinePieceImageRepeat(const NinePieceImage& image)
{
    GCRefPtr<CSSPrimitiveValue> horizontalRepeat, verticalRepeat;
...
static GCRefPtr<CSSValue> valueForNinePieceImage(const NinePieceImage& image, const ComputedStyle& style)
{
    if (!image.hasImage())
    ...
    GCRefPtr<CSSBorderImageSliceValue> imageSlices = valueForNinePieceImageSlice(image);

    // Create the border area slices.
    GCRefPtr<CSSValue> borderSlices = valueForNinePieceImageQuad(image.borderSlices(), style);

    // Create the border outset.
    GCRefPtr<CSSValue> outset = valueForNinePieceImageQuad(image.outset(), style);

    // Create the repeat rules.
    GCRefPtr<CSSValue> repeat = valueForNinePieceImageRepeat(image);

    return createBorderImageValue(imageValue, imageSlices, borderSlices, outset, repeat);
}

static GCRefPtr<CSSValue> valueForReflection(const StyleReflection* reflection, const ComputedStyle& style)
{
    if (!reflection)
        return cssValuePool().createIdentifierValue(CSSValueNone);
...
static GCRefPtr<CSSValueList> valueForItemPositionWithOverflowAlignment(ItemPosition itemPosition, OverflowAlignment overflowAlignment, ItemPositionType positionType)
{
...
static GCRefPtr<CSSValueList> valuesForGridShorthand(const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSlashSeparated();
    for (size_t i = 0; i < shorthand.length(); ++i) {
        GCRefPtr<CSSValue> value = ComputedStyleCSSValueMapping::get(shorthand.properties()[i], style, layoutObject, styledNode, allowVisitedStyle);
        ASSERT(value);
        list->append(value);
    }
    return list;
...
static GCRefPtr<CSSValueList> valuesForShorthandProperty(const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
    for (size_t i = 0; i < shorthand.length(); ++i) {
        GCRefPtr<CSSValue> value = ComputedStyleCSSValueMapping::get(shorthand.properties()[i], style, layoutObject, styledNode, allowVisitedStyle);
...
static GCRefPtr<CSSValueList> valuesForBackgroundShorthand(const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    GCRefPtr<CSSValueList> ret = CSSValueList::createCommaSeparated();
    const FillLayer* currLayer = &style.backgroundLayers();
    for (; currLayer; currLayer = currLayer->next()) {
        GCRefPtr<CSSValueList> list = CSSValueList::createSlashSeparated();
        GCRefPtr<CSSValueList> beforeSlash = CSSValueList::createSpaceSeparated();
        if (!currLayer->next()) { // color only for final layer
            GCRefPtr<CSSValue> value = ComputedStyleCSSValueMapping::get(CSSPropertyBackgroundColor, style, layoutObject, styledNode, allowVisitedStyle);
            ASSERT(value);
            beforeSlash->append(value);
        }
        if (StyleImage *image = currLayer->image())
            beforeSlash->append(image->computedCSSValue());
        else
            beforeSlash->append(cssValuePool().createIdentifierValue(CSSValueNone));
        ...
        GCRefPtr<CSSValueList> afterSlash = CSSValueList::createSpaceSeparated();
...
static GCRefPtr<CSSValueList> valueForContentPositionAndDistributionWithOverflowAlignment(const StyleContentAlignmentData& data)
{
    GCRefPtr<CSSValueList> result = CSSValueList::createSpaceSeparated();
...
static GCRefPtr<CSSPrimitiveValue> valueForLineHeight(const ComputedStyle& style)
{
...
static GCRefPtr<CSSValue> valueForFamily(const AtomicString& family)
{
...
static GCRefPtr<CSSValueList> valueForFontFamily(const ComputedStyle& style)
{
...
static GCRefPtr<CSSPrimitiveValue> valueForFontSize(const ComputedStyle& style)
{
    return zoomAdjustedPixelValue(style.fontDescription().computedSize(), style);
}

static GCRefPtr<CSSPrimitiveValue> valueForFontStretch(const ComputedStyle& style)
{
    return cssValuePool().createValue(style.fontDescription().stretch());
}

static GCRefPtr<CSSPrimitiveValue> valueForFontStyle(const ComputedStyle& style)
{
    return cssValuePool().createValue(style.fontDescription().style());
}

static GCRefPtr<CSSPrimitiveValue> valueForFontVariant(const ComputedStyle& style)
{
    return cssValuePool().createValue(style.fontDescription().variant());
}

static GCRefPtr<CSSPrimitiveValue> valueForFontWeight(const ComputedStyle& style)
{
    return cssValuePool().createValue(style.fontDescription().weight());
}

static GCRefPtr<CSSValue> specifiedValueForGridTrackBreadth(const GridLength& trackBreadth, const ComputedStyle& style)
{
...
static GCRefPtr<CSSValue> specifiedValueForGridTrackSize(const GridTrackSize& trackSize, const ComputedStyle& style)
{
    switch (trackSize.type()) {
    case LengthTrackSizing:
        return specifiedValueForGridTrackBreadth(trackSize.length(), style);
    case MinMaxTrackSizing:
        GCRefPtr<CSSFunctionValue> minMaxTrackBreadths = CSSFunctionValue::create(CSSValueMinmax);
...
static void addValuesForNamedGridLinesAtIndex(const OrderedNamedGridLines& orderedNamedGridLines, size_t i, CSSValueList& list)
{
    ...
    GCRefPtr<CSSGridLineNamesValue> lineNames = CSSGridLineNamesValue::create();
    for (size_t j = 0; j < namedGridLines.size(); ++j)
        lineNames->append(CSSCustomIdentValue::create(namedGridLines[j]));
    list.append(lineNames);
}

static GCRefPtr<CSSValue> valueForGridTrackList(GridTrackSizingDirection direction, const LayoutObject* layoutObject, const ComputedStyle& style)
{
...
static GCRefPtr<CSSValue> valueForGridPosition(const GridPosition& position)
{
    if (position.isAuto())
        return cssValuePool().createIdentifierValue(CSSValueAuto);
...
static GCRefPtr<CSSValue> renderTextDecorationFlagsToCSSValue(int textDecoration)
{
...
static GCRefPtr<CSSValue> valueForTextDecorationStyle(TextDecorationStyle textDecorationStyle)
{
...
static GCRefPtr<CSSValue> touchActionFlagsToCSSValue(TouchAction touchAction)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
...
static GCRefPtr<CSSValue> valueForWillChange(const Vector<CSSPropertyID>& willChangeProperties, bool willChangeContents, bool willChangeScrollPosition)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createCommaSeparated();
...
static GCRefPtr<CSSValue> valueForAnimationDelay(const CSSTimingData* timingData)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createCommaSeparated();
    if (timingData) {
...
static GCRefPtr<CSSValue> valueForAnimationDirection(Timing::PlaybackDirection direction)
{
    switch (direction) {
...
static GCRefPtr<CSSValue> valueForAnimationDuration(const CSSTimingData* timingData)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createCommaSeparated();
    if (timingData) {
...
static GCRefPtr<CSSValue> valueForAnimationFillMode(Timing::FillMode fillMode)
{
    switch (fillMode) {
...
static GCRefPtr<CSSValue> valueForAnimationIterationCount(double iterationCount)
{
...
static GCRefPtr<CSSValue> valueForAnimationPlayState(EAnimPlayState playState)
{
...
static GCRefPtr<CSSValue> createTimingFunctionValue(const TimingFunction* timingFunction)
{
...
static GCRefPtr<CSSValue> valueForAnimationTimingFunction(const CSSTimingData* timingData)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createCommaSeparated();
    if (timingData) {
...
static GCRefPtr<CSSValueList> valuesForBorderRadiusCorner(LengthSize radius, const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
...
static GCRefPtr<CSSValue> valueForBorderRadiusCorner(LengthSize radius, const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> list = valuesForBorderRadiusCorner(radius, style);
    if (list->item(0)->equals(*list->item(1)))
        return GCWrapShared(list->item(0));
    return list;
}

static GCRefPtr<CSSFunctionValue> valueForMatrixTransform(const TransformationMatrix& transform, const ComputedStyle& style)
{
    GCRefPtr<CSSFunctionValue> transformValue;
...
static GCRefPtr<CSSValue> computedTransform(const LayoutObject* layoutObject, const ComputedStyle& style)
{
...
static GCRefPtr<CSSValue> createTransitionPropertyValue(const CSSTransitionData::TransitionProperty& property)
{
...
static GCRefPtr<CSSValue> valueForTransitionProperty(const CSSTransitionData* transitionData)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createCommaSeparated();
    if (transitionData) {
...
static GCRefPtr<CSSValue> valueForContentData(const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
    for (const ContentData* contentData = style.contentData(); contentData; contentData = contentData->next()) {
        if (contentData->isCounter()) {
            ...
            ASSERT(counter);
            GCRefPtr<CSSCustomIdentValue> identifier = CSSCustomIdentValue::create(counter->identifier());
            GCRefPtr<CSSCustomIdentValue> separator = CSSCustomIdentValue::create(counter->separator());
            ...
            GCRefPtr<CSSPrimitiveValue> listStyle = cssValuePool().createIdentifierValue(listStyleIdent);
...
static GCRefPtr<CSSValue> valueForCounterDirectives(const ComputedStyle& style, CSSPropertyID propertyID)
{
...
static GCRefPtr<CSSValue> valueForShape(const ComputedStyle& style, ShapeValue* shapeValue)
{
    if (!shapeValue)
        return cssValuePool().createIdentifierValue(CSSValueNone);
...
static GCRefPtr<CSSValueList> valuesForSidesShorthand(const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
    // Assume the properties are in the usual order top, right, bottom, left.
    GCRefPtr<CSSValue> topValue = ComputedStyleCSSValueMapping::get(shorthand.properties()[0], style, layoutObject, styledNode, allowVisitedStyle);
    GCRefPtr<CSSValue> rightValue = ComputedStyleCSSValueMapping::get(shorthand.properties()[1], style, layoutObject, styledNode, allowVisitedStyle);
    GCRefPtr<CSSValue> bottomValue = ComputedStyleCSSValueMapping::get(shorthand.properties()[2], style, layoutObject, styledNode, allowVisitedStyle);
    GCRefPtr<CSSValue> leftValue = ComputedStyleCSSValueMapping::get(shorthand.properties()[3], style, layoutObject, styledNode, allowVisitedStyle);
...
static GCRefPtr<CSSValueList> valueForBorderRadiusShorthand(const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSlashSeparated();
    ...
    GCRefPtr<CSSValueList> topLeftRadius = valuesForBorderRadiusCorner(style.borderTopLeftRadius(), style);
    GCRefPtr<CSSValueList> topRightRadius = valuesForBorderRadiusCorner(style.borderTopRightRadius(), style);
    GCRefPtr<CSSValueList> bottomRightRadius = valuesForBorderRadiusCorner(style.borderBottomRightRadius(), style);
    GCRefPtr<CSSValueList> bottomLeftRadius = valuesForBorderRadiusCorner(style.borderBottomLeftRadius(), style);

    GCRefPtr<CSSValueList> horizontalRadii = CSSValueList::createSpaceSeparated();
    ...
    GCRefPtr<CSSValueList> verticalRadii = CSSValueList::createSpaceSeparated();
...
GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::valueForShadowData(const ShadowData& shadow, const ComputedStyle& style, bool useSpread)
{
    GCRefPtr<CSSPrimitiveValue> x = zoomAdjustedPixelValue(shadow.x(), style);
    GCRefPtr<CSSPrimitiveValue> y = zoomAdjustedPixelValue(shadow.y(), style);
    GCRefPtr<CSSPrimitiveValue> blur = zoomAdjustedPixelValue(shadow.blur(), style);
    GCRefPtr<CSSPrimitiveValue> spread = useSpread ? zoomAdjustedPixelValue(shadow.spread(), style) : GCRefPtr<CSSPrimitiveValue>();
    GCRefPtr<CSSPrimitiveValue> shadowStyle = shadow.style() == Normal ? GCRefPtr<CSSPrimitiveValue>() : cssValuePool().createIdentifierValue(CSSValueInset);
    GCRefPtr<CSSValue> color = currentColorOrValidColor(style, shadow.color());
    return CSSShadowValue::create(x, y, blur, spread, shadowStyle, color);
}

GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::valueForShadowList(const ShadowList* shadowList, const ComputedStyle& style, bool useSpread)
{
    if (!shadowList)
        return cssValuePool().createIdentifierValue(CSSValueNone);
...
GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::valueForFont(const ComputedStyle& style)
{
    // Add a slash between size and line-height.
    GCRefPtr<CSSValueList> sizeAndLineHeight = CSSValueList::createSlashSeparated();
...
static GCRefPtr<CSSValue> valueForScrollSnapDestination(const LengthPoint& destination, const ComputedStyle& style)
{
    GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
...
static GCRefPtr<CSSValue> valueForScrollSnapPoints(const ScrollSnapPoints& points, const ComputedStyle& style)
{
    if (points.hasRepeat) {
        GCRefPtr<CSSFunctionValue> repeat = CSSFunctionValue::create(CSSValueRepeat);
...
static GCRefPtr<CSSValue> valueForScrollSnapCoordinate(const Vector<LengthPoint>& coordinates, const ComputedStyle& style)
{
...
GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::get(const AtomicString customPropertyName, const ComputedStyle& style)
{
...
GCRefPtr<CSSValue> ComputedStyleCSSValueMapping::get(CSSPropertyID propertyID, const ComputedStyle& style, const LayoutObject* layoutObject, Node* styledNode, bool allowVisitedStyle)
{
    ...
    case CSSPropertyBackgroundColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyBackgroundColor).rgb());
        else
            return currentColorOrValidColor(style, style.backgroundColor());
    ...
    case CSSPropertyBorderTopColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyBorderTopColor).rgb());
        else
            return currentColorOrValidColor(style, style.borderTopColor());
    case CSSPropertyBorderRightColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyBorderRightColor).rgb());
        else
            return currentColorOrValidColor(style, style.borderRightColor());
    case CSSPropertyBorderBottomColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyBorderBottomColor).rgb());
        else
            return currentColorOrValidColor(style, style.borderBottomColor());
    case CSSPropertyBorderLeftColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyBorderLeftColor).rgb());
        else
    ...
    case CSSPropertyWebkitColumnRuleColor:
        if (allowVisitedStyle)
            return cssValuePool().createColorValue(style.visitedDependentColor(CSSPropertyOutlineColor).rgb());
        else
    ...
    case CSSPropertyCursor: {
        ...
        GCRefPtr<CSSValue> value = cssValuePool().createValue(style.cursor());
        if (list) {
    ...
    case CSSPropertyFontFeatureSettings: {
        ...
        for (unsigned i = 0; i < featureSettings->size(); ++i) {
            ...
            GCRefPtr<CSSFontFeatureValue> featureValue = CSSFontFeatureValue::create(feature.tag(), feature.value());
    ...
    case CSSPropertyFontVariantLigatures: {
        ...
        GCRefPtr<CSSValueList> valueList = CSSValueList::createSpaceSeparated();
    ...
    case CSSPropertyAnimation: {
        const CSSAnimationData* animationData = style.animations();
        if (animationData) {
            GCRefPtr<CSSValueList> animationsList = CSSValueList::createCommaSeparated();
            for (size_t i = 0; i < animationData->nameList().size(); ++i) {
                GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
                list->append(CSSCustomIdentValue::create(animationData->nameList()[i]));
    ...
    case CSSPropertyClip: {
        if (style.hasAutoClip())
            return cssValuePool().createIdentifierValue(CSSValueAuto);
        GCRefPtr<CSSPrimitiveValue> top = style.clip().top().isAuto()
            ? cssValuePool().createIdentifierValue(CSSValueAuto)
            : zoomAdjustedPixelValue(style.clip().top().value(), style);
        GCRefPtr<CSSPrimitiveValue> right = style.clip().right().isAuto()
            ? cssValuePool().createIdentifierValue(CSSValueAuto)
            : zoomAdjustedPixelValue(style.clip().right().value(), style);
        GCRefPtr<CSSPrimitiveValue> bottom = style.clip().bottom().isAuto()
            ? cssValuePool().createIdentifierValue(CSSValueAuto)
            : zoomAdjustedPixelValue(style.clip().bottom().value(), style);
        GCRefPtr<CSSPrimitiveValue> left = style.clip().left().isAuto()
            ? cssValuePool().createIdentifierValue(CSSValueAuto)
    ...
    case CSSPropertyTransition: {
        ...
        if (transitionData) {
            GCRefPtr<CSSValueList> transitionsList = CSSValueList::createCommaSeparated();
            for (size_t i = 0; i < transitionData->propertyList().size(); ++i) {
                GCRefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
                list->append(createTransitionPropertyValue(transitionData->propertyList()[i]));
    ...
    case CSSPropertyBorder: {
        GCRefPtr<CSSValue> value = get(CSSPropertyBorderTop, style, layoutObject, styledNode, allowVisitedStyle);
