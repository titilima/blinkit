...
// -------------------------------------------------
//   File Name: ScriptedIdleTaskController.cpp
...
ScriptedIdleTaskController::ScriptedIdleTaskController(ExecutionContext* context)
    ...
    // BKTODO:, m_scheduler(Platform::current()->currentThread()->scheduler())
    , m_nextCallbackId(0)
    , m_suspended(false)
{
    suspendIfNeeded();
...
{
    ASSERT(false); // BKTODO: visitor->trace(m_callbacks);
...
void ScriptedIdleTaskController::cancelCallback(CallbackId id)
{
    ...
    ASSERT(false); // BKTODO: m_callbacks.remove(id);
}
...
{
    ASSERT(false); // BKTODO: m_callbacks.clear();
}
...
{
    ASSERT(false); // BKTODO: return !m_callbacks.isEmpty();
    return false;
}
