...
#include "./PaintLayerScrollableArea.h"

#include "blinkit/blink/public/platform/Platform.h"
#include "blinkit/blink/public/platform/WebCompositorMutableProperties.h"
...
#include "blinkit/blink/renderer/core/paint/PaintLayerFragment.h"
...
GCRefPtr<Scrollbar> PaintLayerScrollableArea::ScrollbarManager::createScrollbar(ScrollbarOrientation orientation)
{
    ASSERT(orientation == HorizontalScrollbar ? !m_hBarIsAttached : !m_vBarIsAttached);
    GCRefPtr<Scrollbar> scrollbar;
    ...
    if (hasCustomScrollbarStyle) {
        scrollbar = LayoutScrollbar::createCustomScrollbar(&m_scrollableArea, orientation, actualLayoutObject.node());
    } else {
        ...
        scrollbar = Scrollbar::create(&m_scrollableArea, orientation, scrollbarSize, &m_scrollableArea.box().frame()->page()->chromeClient());
...
{
    GCRefPtr<Scrollbar>& scrollbar = orientation == HorizontalScrollbar ? m_hBar : m_vBar;
    ASSERT(orientation == HorizontalScrollbar ? !m_hBarIsAttached: !m_vBarIsAttached);
    if (!scrollbar)
        return;
